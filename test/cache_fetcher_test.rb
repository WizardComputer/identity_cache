...
class CacheFetcherTest < IdentityCache::TestCase
  ...
  def test_fetch_lock_wait_limit_reached
    ...
    lock = write_lock(client_id: SecureRandom.uuid, data_version: data_version)
  ...
  def write_lock(client_id:, data_version:)
    lock = IdentityCache::CacheFetcher::FillLock.build(client_id: SecureRandom.uuid, data_version: data_version)
  ...
  def unconnected_cache_backend
    ActiveSupport::Cache::MemcachedStore.new("127.0.0.1:#{open_port}", support_cas: true)
  end
